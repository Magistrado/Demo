...
class StubDBHandler(DBHandler):
    ...
    def create_category(self, breadcrumb, comp_img=None):
        return FakeObjectOscar()

    def get_subcomponent_class(self):
        return FakeObjectOscar()

    def asign_prod_replacement(self, p_origin, p_asign):
        pass

    def get_create_prods_attrib(self, p):
        return (FakeObjectOscar(), FakeObjectOscar(), FakeObjectOscar())

    def create_prods(self, cat, is_aval, prod_name,
...
class MockDB(DBHandler):
    ...
    def get_create_prods_attrib(self, product_class):
        pass
...
class TestUnitExtraerCats(unittest.TestCase):
    ...
    def test_unElementoTodosNiveles_Cierto(self):
        ...
        resul = categorizer.extract_cats(self.handler.read('prueba.json'))
...
class CreaProdsTest(unittest.TestCase):
    ...
    def test_productos_sin_partnumber__regresaRunTimeError(self):
        self.assertRaises(RuntimeError, self.realDB.create_prods, self.cat, True, 'Hey', None, 'Man', 'ori',
                          '1', '0.00', '0.00', '0.00')

    def test_productos_partnumber_string_empty__regresaRunTimeError(self):
        self.assertRaises(RuntimeError, self.realDB.create_prods, self.cat, True, 'Hey', '', 'Man', 'ori',
                          '1', '0.00', '0.00', '0.00')

    def test_agregarDiagNumMayor5Caracteres_LevantaDatabaseError(self):
        self.assertRaises(DatabaseError, self.realDB.create_prods, self.cat, True, 'Prod', '1', 'Man', 'Origin'
                          , 'pppppp', '0.00', '0.00', '0.00')

    def test_incluirPreciosPriceExclTaxComa_OK(self):
        prod = self.realDB.create_prods(self.cat, True, 'Prod', '1', 'Man', 'Origin'
    ...
    def test_incluirPreciosPriceExclTaxComa_regresaInvalidOperation(self):
        self.assertRaises(InvalidOperation, self.realDB.create_prods, self.cat, True, 'Prod', '1', 'Man', 'Origin'
                          , 'pppppp', '', '0.00', '0.00')
...
class TestExtraerProds(unittest.TestCase):
    ...
    def test_variasPartesRepetidas(self,):
        nro_prod = categorizer.extract_prods(casos.productos_repetidos_categorias, self.mockDB)
...
class TestIntegrationExtraerProds(unittest.TestCase):
    ...
    def test_crear_productos_regresa6(self):
        nro_prod = categorizer.extract_prods(casos.productos_repetidos_categorias, self.realDB)
        self.assertEqual(nro_prod, 6)
...
class TestIntegrationDB_NavProds(unittest.TestCase):
    ...
    def test_varios_prods_cruzados_returnsTrue(self):
        nro_recom = categorizer.nav_prods(casos.varios_productos_con_reemplazado_cruzado, ["Hola", "Prueba"], self.realDB)
